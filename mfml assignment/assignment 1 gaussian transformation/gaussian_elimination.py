...
def check_dimensions(A, b):
    ...
    if len(A[0]) == 0:
        raise ValueError("the coefficient matrix must have at least one column\n")
...
def print_parametric_solution(aug, pivot_columns, num_variables, tolerance=1e-10):
    """
    Formulates and displays the general algebraic solution in parametric form.
    Basic variables are expressed strictly in terms of free parameters (t1, t2, ...).
    """
    free_columns = [j for j in range(num_variables) if j not in pivot_columns]
    
    param_names = {col: f"t{idx+1}" for idx, col in enumerate(free_columns)}

    var_coeffs = {v: {col: 0.0 for col in free_columns} for v in range(num_variables)}
    for v in range(num_variables):
        var_coeffs[v]['const'] = 0.0

    for col in free_columns:
        var_coeffs[col][col] = 1.0

    for i in range(len(pivot_columns) - 1, -1, -1):
        p_col = pivot_columns[i]
        pivot = aug[i][p_col]
        
        var_coeffs[p_col]['const'] = aug[i][num_variables] / pivot
        
        for j in range(p_col + 1, num_variables):
            factor = aug[i][j] / pivot
            if abs(factor) > tolerance:
                var_coeffs[p_col]['const'] -= factor * var_coeffs[j]['const']
                for free_col in free_columns:
                    var_coeffs[p_col][free_col] -= factor * var_coeffs[j][free_col]

    print("\nParametric Form of General Solution:")
    
    for j in range(num_variables):
        if j not in pivot_columns:
            print(f"x{j+1} = {param_names[j]}  (Free variable, any real number)")
        else:
            terms = []
            const_val = var_coeffs[j]['const']
            
            if abs(const_val) > tolerance:
                terms.append(f"{const_val:.6f}")

            for free_col in free_columns:
                coeff = var_coeffs[j][free_col]
                if abs(coeff) > tolerance:
                    sign = "+" if (coeff > 0 and terms) else ("-" if coeff < 0 else "")
                    mag = abs(coeff)
                    mag_str = f"{mag:.6f} * " if abs(mag - 1.0) > tolerance else ""
                    
                    param_str = f"{sign} {mag_str}{param_names[free_col]}".strip()
                    terms.append(param_str)

            if not terms:
                print(f"x{j+1} = 0.000000")
            else:
                print(f"x{j+1} = {' '.join(terms)}")
...
def gaussian_elimination(A, b):
    ...
    try:
        ...
        if rank == variables:
            ...
            print("\nResult: system has INFINITELY MANY solutions")
            print(f"System has {variables - rank} free variable(s).")
            
            print_parametric_solution(aug, pivot_columns, variables)

            x_particular = back_substitution(aug, pivot_columns, variables)
            print("\nOne Particular Solution (setting free parameters = 0):")
            for i in range(variables):
                print(f"x{i+1} = {x_particular[i]:.6f}")

            verify_soln(original_A, original_b, x_particular)
            return x_particular
